	  	...
	  	3|03) #input
				...
				case ${mode} in
					...
					${MODE_NONINTERACTIVE})
						input=$1
						shift
						if [ "${input}" == "" ] ; then echo "ERROR: MISSING INPUT!" >&2 ; echo "Index: ${arrayindex} Array: ${array[*]}" >&2 ; fi
					;;
				esac
			...
			for k in {5..9} ; do
				...
				for l in {5..9} ; do
					...
					for m in {5..9} ; do
						...
						echo 0 > ${testing_loop}.loop
						echo -ne "\rTesting sequence: (${phase_sequence[*]})"
						for phase_setting in ${phase_sequence[*]} ; do
							load_array_params ${input_file}
							if [ ${testing_loop} -eq 0 ] ; then
								run_script ${phase_setting} 0 >> ${testing_loop}.loop
							else
								run_script ${phase_setting} < <(tail -f $(( testing_loop - 1 )).loop) >> ${testing_loop}.loop
							fi
							echo -n "  Result: ${result}"
						done
						if [ ${result} -gt ${best_result} ] ; then
							echo "   New best!  Old best: ${best_result}"
							best_result=${result}
						fi
					done
				done
...
echo "Test1: "
main btest1.txt
echo "Test1 execution complete!"

echo "Test2: "
main btest2.txt
echo "Test2 execution complete!"
